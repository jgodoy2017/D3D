...
void X11Operations::drawImage(Display* dpy, Window w, GC gc, int lWidth, int lHeight, int depth, int* image, int lMinVal, int lMaxVal, int lWhite){
	...
	for(int px = 0; px < lWidth * lHeight; px++){
		float mappedValue = (lWhite/(lMaxVal-lMinVal)) * (image[px]-lMinVal);  // Interpolacion lineal + escalado (por temas visuales).
		unsigned char clippedValue = static_cast<unsigned char>(mappedValue) % 256;
